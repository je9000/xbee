...
use constant XBEE_API_TYPE__ZIGBEE_TRANSMIT_REQpackage Device::XBee::API;
...
sub __data_to_int
{
    ...
    my $data_as_int;
    if ( length($data) == 1 ) {
        $data_as_int = unpack( 'C', $data );
    } elsif ( length( $data ) == 2 ) {
        $data_as_int = unpack( 'n', $data );
    } elsif ( length( $data ) == 4 ) {
        $data_as_int = unpack( 'N', $data);
    } elsif ( length( $data ) == 8 ) {                   
...
sub __parse_at_command_response {
    ...
    if ( $r->{command} eq 'ND' ) {
        ...
        if ( length( $r->{data} ) == 1 ) {
            $r->{data_as_int} = unpack( 'C', $r->{data} );package Device::XBee::API;
